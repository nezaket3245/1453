        ...
        <>
            ...
            >
                ...
                >
                        ...
                        {navigationItems.map((item) => (
                            <li key={item.href} className="relative" role="none" onMouseEnter={() => item.children && setActiveDropdown(item.href)} onMouseLeave={() => setActiveDropdown(null)}>
                                <Link
                                    ...
                                    className="px-4 py-2 rounded-lg font-bold text-neutral-900 hover:text-primary-600 hover:bg-primary-50 transition-all flex items-center gap-1 text-[15px]"
                                    role="menuitem"
                                                ...
                                                <ul className="bg-white rounded-xl shadow-xl border p-2 min-w-[240px]" role="menu" aria-label={`${item.label} alt menüsü`}>
                                                    {item.children.map((child) => (
                                                        <li key={child.href} role="none">
                                                            <Link href={child.href} title={child.label} className="block px-4 py-2.5 text-neutral-800 font-semibold hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" role="menuitem">
                                                                {child.label}
                                                            </Link>
                                                        </li>
                                                    ))}
                    ...
                    {/* Mobile Toggle */}
                    <button
                        className="lg:hidden p-2 min-w-[48px] min-h-[48px] text-primary-600"
...
function ChevronDownIcon({ className }: { className?: string }) {
    return <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>;
}
function MenuIcon({ className }: { className?: string }) {
    return <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16m-7 6h7" /></svg>;
}
