...
import { glassSystems, getSystemBySlug, getRelatedSystems } from "@/lib/camBalkonData";

interface CamBalkonPageProps {
...
export async function generateMetadata({ params }: CamBalkonPageProps): Promise<Metadata> {
    ...
    return {
        ...
        alternates: { canonical: `https://egepenakcayapi.com/cam-balkon-sistemleri/${slug}` },
    };
}

export default async function CamBalkonProductPage({ params }: CamBalkonPageProps) {
    const { slug } = await params;
    const system = getSystemBySlug(slug);
    if (!system) notFound();
        ...
        <>
            ...
            <main id="main-content" className="min-h-screen bg-neutral-50">
                <PageHero
                    ...
                    compact
                    breadcrumbs={[
                        { label: "Cam Balkon Sistemleri", href: "/cam-balkon-sistemleri" },
                        { label: system.name },
                    ]}
                />
                ...
                <div className="container-custom py-10 md:py-14">
                    ...
                    {relatedSystems.length > 0 && (
                        <div className="mt-12 pt-8 border-t border-neutral-200">
                            <h2 className="text-xl font-bold text-neutral-900 mb-6">Diğer Cam Balkon Sistemleri</h2>
                            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {relatedSystems.map(s => (
                                    <Link key={s.id} href={`/cam-balkon-sistemleri/${s.slug}`} className="group block bg-white rounded-xl border border-neutral-200 overflow-hidden hover:shadow-md transition-all">
